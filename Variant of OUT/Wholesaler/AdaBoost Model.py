...
sheet_R = pd.read_excel(r'C:\Users\rahul\Project\Variant of OUT Policy\supply_chain_results_2.xlsx', sheet_name = 'Wholesaler')
sheet_R = sheet_R[['Replenishment quantity','Beginning inventory','Inventory position', 'Retailer order','Allocated quantity', 'Ending inventory', 'Forecast', 'Order up to level', 'EDR', 'Order quantity', 'Lost sales']]
...
with open('W_ADB_final_model_pruned.pkl', 'wb') as file:
...
with open('W_ADB_final_model_pruned.pkl', 'rb') as file:
...
import math
input_data = pd.DataFrame({'Beginning inventory': [36], 'Inventory position': [36],'Forecast': [17], 'Order up to level': [51]})
