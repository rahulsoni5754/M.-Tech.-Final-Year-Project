...
sheet_F = pd.read_excel(r'C:\Users\rahul\Project\Variant of OUT policy\supply_chain_results_2.xlsx', sheet_name = 'Factory')
sheet_F = sheet_F[['Replenishment quantity','Beginning inventory','Inventory position', 'Distributor order','Allocated quantity', 'Ending inventory', 'Forecast', 'Order up to level', 'EDR', 'Order quantity', 'Lost sales']]
...
p = data[['Replenishment quantity','Beginning inventory','Inventory position', 'Distributor order','Allocated quantity', 'Ending inventory', 'Forecast', 'Order up to level', 'EDR', 'Lost sales']]
...
x = data[['Beginning inventory','Inventory position',  'Forecast', 'Order up to level']]
y = data[['Order quantity']]
...
with open('F_LGB_final_model_pruned.pkl', 'wb') as file:
...
with open('F_LGB_final_model_pruned.pkl', 'rb') as file:
...
import math
r= int(input(" replishment quantity   "))
b= int(input(" beginning inventory  "))
i= int(input(" inventory position  "))
f= int(input("forecast  "))
o= int(input(" order upto level  "))
e= int(input(" EDR  "))
input_data = pd.DataFrame({'Replenishment quantity': [r], 'Beginning inventory': [b],'Inventory position': [i],'Forecast': [f],'Order up to level': [o],'EDR': [e]})
