...
sheet_R = pd.read_excel(r'C:\Users\rahul\OneDrive\Desktop\Project Work\S4\S4 Final Evaluation\Comparative Analysis\Excel Simulation (OUT).xlsx', sheet_name = 'Sheet_R')
sheet_W = pd.read_excel(r'C:\Users\rahul\OneDrive\Desktop\Project Work\S4\S4 Final Evaluation\Comparative Analysis\Excel Simulation (OUT).xlsx', sheet_name = 'Sheet_W')
sheet_D = pd.read_excel(r'C:\Users\rahul\OneDrive\Desktop\Project Work\S4\S4 Final Evaluation\Comparative Analysis\Excel Simulation (OUT).xlsx', sheet_name = 'Sheet_D')
sheet_F = pd.read_excel(r'C:\Users\rahul\OneDrive\Desktop\Project Work\S4\S4 Final Evaluation\Comparative Analysis\Excel Simulation (OUT).xlsx', sheet_name = 'Sheet_F')



with open(r'C:\Users\rahul\Project\OUT Policy\Retailer Stage\R_ADB_final_model_pruned.pkl', 'rb') as file:
    retailer_model = pickle.load(file)
    
with open(r'C:\Users\rahul\Project\OUT Policy\Wholesaler stage\W_ADB_final_model_pruned.pkl', 'rb') as file:
    wholesaler_model = pickle.load(file)

with open(r'C:\Users\rahul\Project\OUT Policy\Distributor Stage\D_ADB_final_model_pruned.pkl', 'rb') as file:
    distributor_model = pickle.load(file)

with open(r'C:\Users\rahul\Project\OUT Policy\Factory Stage\F_ADB_final_model_pruned.pkl', 'rb') as file:
...
data_retailer = {'Period': [],'Replenishment quantity': [],'Beginning inventory': [],'Inventory position': [],'Customer order': [],
                 'Allocated quantity': [],'Ending inventory': [],'Forecast': [],'Order up to level': [],'Order quantity': [],'Lost sales': []}
...
while t <= Maxit:
    t += 1
    Demand_t = int(np.random.normal(20, 5))
    Demand_t = max(0, Demand_t)
    ...
    Retailer_features = pd.DataFrame([[Retailer_beginning_inventory, Retailer_inventory_position, Retailer_forecast, Retailer_order_up_to_level]], columns=['Retailer_beginning_inventory', 'Retailer_inventory_position', 'Retailer_forecast', 'Retailer_order_up_to_level'])
    ...
    Wholesaler_features = pd.DataFrame([[Wholesaler_inventory_position, Wholesaler_forecast, Wholesaler_order_up_to_level]], columns=['Wholesaler_inventory_position', 'Wholesaler_forecast', 'Wholesaler_order_up_to_level'])
    ...
    Factory_features = pd.DataFrame([[Factory_inventory_position, Factory_forecast, Factory_order_up_to_level]], columns=['Factory_inventory_position', 'Factory_forecast', 'Factory_order_up_to_level'])
...
with pd.ExcelWriter('OUT_testing_results_6.xlsx') as writer:
...
#load the excel file    
excel_file = pd.ExcelFile('OUT_testing_results_6.xlsx')
#print(excel_file)
