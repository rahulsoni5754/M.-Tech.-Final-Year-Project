...
sheet_R = pd.read_excel(r'C:\Users\rahul\Project\OUT policy\supply_chain_results_1.xlsx', sheet_name = 'Retailer')
sheet_R = sheet_R[['Replenishment quantity','Beginning inventory','Inventory position', 'Customer order','Allocated quantity', 'Ending inventory', 'Forecast', 'Order up to level', 'Order quantity', 'Lost sales']]
...
p = data[['Replenishment quantity','Beginning inventory','Inventory position', 'Customer order','Allocated quantity', 'Ending inventory', 'Forecast', 'Order up to level', 'Lost sales']]
...
x = data[['Replenishment quantity','Beginning inventory','Inventory position', 'Customer order','Allocated quantity', 'Ending inventory', 'Forecast', 'Order up to level', 'Lost sales']]
...
plt.scatter(y_test_actual[0:100], y_pred_actual[0:100], alpha=1)
plt.plot([min(y_test_actual[0:100]), max(y_test_actual[0:100])], [min(y_test_actual[0:100]), max(y_test_actual[0:100])], '-', color='red')
...
import math
input_data = pd.DataFrame({'Replenishment quantity': [14],'Beginning inventory': [36],'Inventory position': [36],'Forecast': [17],'Order up to level': [51]})
