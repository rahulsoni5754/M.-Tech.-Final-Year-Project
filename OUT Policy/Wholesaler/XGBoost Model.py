...
x = data[['Replenishment quantity', 'Beginning inventory', 'Inventory position', 'Allocated quantity', 'Forecast', 'Order up to level']]
y = data[['Order quantity']]
...
import math
input_data = pd.DataFrame({'Beginning inventory': [14],'Inventory position': [36],'Retailer order': [11],'Forecast': [17],'Order up to level': [51]})
print(math.ceil(scaler_y.inverse_transform(final_model_pruned.predict(scaler_x.transform(input_data)).reshape(-1,1))))
