...
x_train_pruned = scaler_x.fit_transform(x_train_pruned)
x_test_pruned = scaler_x.transform(x_test_pruned)
...
with open('D_XGB_final_model_pruned.pkl', 'wb') as file:
...
with open('D_XGB_final_model_pruned.pkl', 'rb') as file:
...
import math
input_data = pd.DataFrame({'Beginning inventory': [59], 'Inventory position': [59],'Wholesaler order': [0], 'Allocated quantity': [0], 'Ending inventory': [59],'Forecast': [29], 'Order up to level': [87]})
